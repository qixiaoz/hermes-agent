...
class HonchoSessionManager:
    ...
    def __init__(
        ...
        self._config = config
        # Capture the profile-owned fallback directory while this manager is
        # constructed under the request/profile context.  ContextVar overrides
        # do not propagate to the background writer thread, so resolving the
        # Hermes home there can silently redirect failed writes to the launch
        # profile instead.
        self._fallback_dir = get_hermes_home() / "honcho_fallback"
